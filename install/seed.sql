...
('impressum', 'Impressum / Imprint', '<section><h3>Impressum / Imprint</h3><p>Information pursuant to legal notice requirements:</p><address><strong>Website Owner:</strong><br>{{owner_name}}<br>{{owner_address}}<br><br>E-Mail: <a href="mailto:{{owner_email}}">{{owner_email}}</a></address></section>', 'Legal notice and imprint.'),
