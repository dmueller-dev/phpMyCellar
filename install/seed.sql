...
('welcome', 'Homepage: Welcome Introduction', '<p>Welcome to <strong>{{site_name}}</strong>! This is an open-source fine wine cellar notebook and management system. Here, you can catalog your wine bottles, keep track of storage locations and orders, and record thorough tasting notes and ratings.</p><p>You can edit this content directly from your admin backend under <em>Site Settings &gt; Static Pages</em>.</p>', 'Welcome to phpMyCellar fine wine notebook.'),
('get_in_touch', 'Homepage: Get in Touch Card', '<p>If you have any questions, feedback, or would like to request tasting access to my cellar notebook, please feel free to reach out directly via email at <a href="mailto:{{owner_email}}">{{owner_email}}</a>.</p>', 'Contact and cellar notebook access card.'),
('impressum', 'Impressum / Imprint', '<h3>Impressum / Imprint</h3><p>Information pursuant to legal notice requirements:</p><p><strong>Website Owner:</strong><br>{{owner_name}}<br>{{owner_address}}<br><br>E-Mail: <a href="mailto:{{owner_email}}">{{owner_email}}</a></p>', 'Legal notice and imprint.'),
('privacy', 'Privacy Policy', '<h3>Privacy Policy</h3><p>This website uses a session cookie only when an authorised user logs in. The session cookie is essential for authentication and ensures you remain logged in during your session. The cookie does not store any personal information and is automatically deleted when you close your browser.</p><p><strong>No</strong> cookies or tracking scripts are used for non-logged-in visitors. We do not use third-party analytics, advertisements, or tracking services.</p><p>For registered members, only minimal personal information (name, email address) is stored on the server for authentication and notification purposes.</p>', 'Privacy policy for phpMyCellar.'),
('tnotes_sidebar', 'Tasting Notes: Sidebar Introduction', '<p>This is a list of all the tasting notes I have published. Keep in mind that this is not my main job, but a hobby. However, I usually manage to write at least one new note per week.</p><h4>How I rate wines</h4><p>I rate wines on my personal <strong>20-point DM scale</strong>. On this scale, I rate the <em>absolute</em> quality of wines. My ratings may appear low relative to those of most popular wine reviewers at first glance, but I make use of the entire range from 0 to 20 points:</p><table><tr><td style="width:70px">20</td><td>one-of-a-kind</td></tr><tr><td>17-19</td><td>grand vin</td></tr><tr><td>13-16</td><td>excellent</td></tr><tr><td>9-12</td><td>very good</td></tr><tr><td>5-8</td><td>good</td></tr><tr><td>3-4</td><td>passable</td></tr><tr><td>1-2</td><td>subpar</td></tr><tr><td>0</td><td>poor</td></tr></table><p>As you can see, &quot;good&quot; wines start at 5 points already!</p><p><a href="/blog.php?id=26" title="How I rate wines">Find out more about how I rate wines.</a></p>', 'Introductory text and 20-point rating guide shown in the right sidebar of the tasting notes index.'),
('wines_sidebar', 'Wine Database: Sidebar Notice', '<p>This is not a list of my wine cellar. These are wines I\'ve written about in a tasting note or in a story on my blog.</p>', 'Notice displayed in the sidebar on the wines index.'),
('winemenu_sidebar', 'Wine Menu: Cellar Invitation Sidebar', '<p>While there are more bottles lying in my cellar, these are all the wines that are ready to be drunk today. <strong>Every wine</strong> can be opened, so please feel free to choose any wine you like! These wines were collected to be shared and enjoyed in the company of friends and family.</p>', 'Invitation text shown in the wine menu sidebar.'),
('vintages_sidebar', 'Vintage Reports: Sidebar Overview & Note', '<h3 style="margin-top:0;">About vintage reports</h3><p style="font-size:small; line-height:1.4;">Vintage scores reflect my personal tasting notes on the wines reviewed. They are updated dynamically as new tasting notes are posted.</p><p style="font-size:small;"><a href="/tnotes.php">Browse all tasting notes &rarr;</a></p>', 'Overview and notes shown in the vintage reports sidebar.'),
('blog_sidebar', 'Stories: Sidebar Introduction', '<p>In this section, I don\'t usually write about individual wines, but about larger tastings of several wines (e.g. horizontals, verticals, etc.) or personal experiences on wine trips and at restaurants.</p><p>If a report refers to a specific wine, this is usually linked so that you can quickly find more information about the wine in question on my site.</p>', 'Introduction displayed in the sidebar of the stories index.');
